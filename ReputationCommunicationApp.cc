...
void ReputationCommunicationApp::onWSM(BaseFrame1609_4* frame)
{
    ...
    EV << "Sender " << nodeId << " has a reputation score of: " << senderScore << endl;
      ...
      if (senderScore >= Threshold_Score && isCertificateValid) {
          ...
          if (isHonest) {
              ...
              // Change route if on accident route
              std::string accidentRoute = wsm->getDemoData();
              ...
              forwardMessage(wsm);
              forwardedMessages++;
              // Do not change the route since it's a false message
              EV << "Route remains unchanged despite message from malicious vehicle " << nodeId << endl;
          }

      } else {
          bool isHonest = isHonestVehicle(nodeId);
            if (!isCertificateValid || senderScore < Threshold_Score) {
                if (isHonest) {
    ...
    void ReputationCommunicationApp::changeRoute(std::string accidentRoute) {
        try {
            traciVehicle->changeRoute("-28822164#4", 9999);  // Adjust route ID if necessary
            EV << "Route changed to avoid accident on route: " << accidentRoute << endl;
        ...
        sendDown(wsm->dup());
                ...
                void ReputationCommunicationApp::sendDENMMessage()
                {
                  ...
                  ReputationCommunicationAppMessage* wsm = new ReputationCommunicationAppMessage();
                   ...
                   if (isHonestVehicle(MNodeId)) {
                    ...
                    EV << "DENM message created by Vehicle " << MNodeId << " with reputation score " << myScore << endl;
                ...
                void ReputationCommunicationApp::simulateSybilAttack()
                {
                    for (int i = 0; i < 5; ++i) {
                        ...
                        UpdateReputationKnowledge(MNodeId, reputationScore);
